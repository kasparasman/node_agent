import { VideoSource, LocalVideoTrack, TrackPublishOptions, VideoCodec, TrackSource, VideoFrame, VideoBufferType, } from '@livekit/rtc-node';
 ...
 */
export async function startVideoRelay(shared) {
    ...
    const options = new TrackPublishOptions({
        ...
        videoEncoding: { maxFramerate: FPS, maxBitrate: 500000n },
    ...
    setInterval(() => {
        if (shared.useDummyVideo) {
            ...
            if (shared.videoFrameBuffer && shared.videoFrameBuffer.length > 0) {
                ...
                const bufferedFrame = shared.videoFrameBuffer.shift();
                if (bufferedFrame) {
                    const realFrame = new VideoFrame(bufferedFrame.buffer, bufferedFrame.width, bufferedFrame.height, VideoBufferType.I420);
                ...
                const realFrame = new VideoFrame(shared.latestFrame.buffer, shared.latestFrame.width, shared.latestFrame.height, VideoBufferType.I420);
                source.captureFrame(realFrame);
