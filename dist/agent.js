...
import { LocalVideoTrack, VideoSource, TrackSource, AudioFrame, AudioSource, LocalAudioTrack } from '@livekit/rtc-node';
//@ts-ignore
...
async function waitForVideoOrStreamReady(timeout = 5000) {
...
// ====================================================
async function connectToDIDWebSocket(url, token) {
...
function sendDIDMessage(ws, message) {
...
// ====================================================
function onStreamEvent(messageEvent) {
...
async function createPeerConnection(ctx, offer, iceServers) {
    if (!peerConnection) {
                    ...
                    audioSink.ondata = (data) => {
                        try {
                            ...
                            if (data.sampleRate !== sampleRate || data.channelCount !== channelCount) {
                                if (canLogMismatch) {
                                    console.warn(`[Audio Config Mismatch] Expected sampleRate=${sampleRate}, channelCount=${channelCount} but received sampleRate=${data.sampleRate}, channelCount=${data.channelCount}. Skipping frame.`);
                            ...
                            const audioFrame = new AudioFrame(data.samples, data.sampleRate, data.channelCount, data.numberOfFrames);
...
// ====================================================
async function getRetrievedContext(queryText) {
    let vector = [];
    try {
        ...
        const denseEmbedding = embedResponse.data[0];
    ...
    entry: async (ctx) => {
        ...
        try {
            ...
            ctx.room.on("dataReceived", (data) => {
                try {
                    const jsonString = (data instanceof Buffer ? data.toString() : new TextDecoder().decode(new Uint8Array(data)));
